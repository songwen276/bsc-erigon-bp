...
// NextNextInTurnValidator return the next in-turn validator for header
func (p *Parlia) NextNextInTurnValidator(chain consensus.ChainHeaderReader, header *types.Header) (common.Address, error) {
	snap, err := p.snapshot(chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return common.Address{}, err
	}

	return snap.nextNextValidator(), nil
}
